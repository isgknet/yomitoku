...
- 📄 多様な出力形式をサポートしています。html やマークダウン、json、csv のいずれかのフォーマットに変換可能です。また、文書内に含まれる図表、画像の抽出の出力も可能です。文書画像をサーチャブルPDFに変換する処理もサポートしています。さらに、画像ベースのEPUB（自炊EPUB等）を入力としてサーチャブルEPUBに変換する機能、およびGradioベースのWeb GUIも提供しています。
...
- 2026 年  5 月  6 日 サーチャブルEPUB出力 / GradioベースWeb GUI を追加
...
| `--format`(`-f`) | 出力形式のファイルフォーマットを指定します。(json, csv, html, md, pdf(searchable-pdf), epub(searchable-epub) をサポート。`.epub` 入力時は `-f` 未指定でも自動的に `epub` 出力) |
...
画像ベースのEPUB（各ページが1枚の画像で構成された「自炊」EPUB等）を入力として、元のレイアウト・CSS・TOC・spineを保ったまま、透明なテキストレイヤを重ねたサーチャブルEPUBに変換できます。テキスト埋め込みの無いEPUBでも、検索やコピー＆ペーストが可能になります。

```bash
# 入力が .epub の場合は -f を省略しても自動的に epub 出力
yomitoku input.epub -o output.epub

# 明示指定
yomitoku input.epub -o output.epub -f epub
```

**処理仕様**:

- 単一のラスタ画像のみを含むspineページのみOCR対象。テキスト共存・複数画像・SVG・画像なしページは元バイトのまま出力EPUBに保持されます。
- 透明テキストは `<span class="word">` を絶対配置（% 座標 + `cqw` フォントサイズ）で重ね、縦書きは1文字1spanで配置します。
- M+ TTFフォントは既定でEPUBに同梱されます。`--epub_no_embed_font` で同梱を無効化、`--font_path` で別TTF指定可。
- 出力先は `-o foo.epub` のようにファイル名指定、または `-o ./out` のようにディレクトリ指定の双方をサポート。

## 🖥️ Web GUI (Gradio)

Gradioベースのブラウザ向けGUIから、画像 / PDF / EPUB をアップロードして、json / html / markdown / searchable-pdf / searchable-epub の各形式で出力できます。

```bash
pip install yomitoku[gui]
yomitoku_gui
# → http://127.0.0.1:7860
```
...
| `--share` | Gradio公開URLを生成（外部からアクセス可能） |

GUI上で device（cuda/cpu）、軽量モード、最大ページ数の制限を切り替え可能。OCR モデルは初回のみロードされ、以降はリクエスト毎に再利用されます。
